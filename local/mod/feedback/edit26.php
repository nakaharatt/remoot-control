    ...
    $add_item_form->display();
		...
		// BYOD links
		 if (!$secId = $DB->get_field('course_modules', 'section', array('id' => $id))) {
			...
			//echo $OUTPUT->heading(get_string('byod_urls', 'byod'));
			echo get_string('byod_urls', 'byod')."<br/>";
			$message = get_string('teacher', 'byod').' : <a href="'.$CFG->wwwroot . '/BYOD/fbremote.php?sectionid=' . $secId.'" target="blank" >';
			$message .= $CFG->wwwroot . '/BYOD/fbremote.php?sectionid=' . $secId;
			$message .= '</a><br/>';
			$message .= get_string('student', 'byod').' : <a href="'.$CFG->wwwroot . '/BYOD/fbstudentRemote.php?sectionid=' . $secId.'" target="blank" >';
			$message .= $CFG->wwwroot . '/BYOD/fbstudentRemote.php?sectionid=' . $secId;
			$message .= '</a><br/><br/>';
			echo $message;
		}
		//
