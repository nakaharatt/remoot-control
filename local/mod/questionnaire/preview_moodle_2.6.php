...
if ($id) {
    ...
    // klermor | 20140603
     if (!$secId = $DB->get_field('course_modules', 'section', array('id' => $id))) {
        ...
        $message = "URL pour BYOD : ";
        $message .= '<a href="'.$CFG->wwwroot . '/BYOD/?sectionid=' . $secId.'" target="blank" >';
        $message .= $CFG->wwwroot . '/BYOD/?sectionid=' . $secId;
        $message .= '</a>';
    }
    //
} else {
...
// klermor
$questionnaire->survey_print_render($message, 'preview', $course->id, $rid = 0, $popup);
